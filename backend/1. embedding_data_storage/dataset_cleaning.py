...
query = {
    "_source": False,
    "query": {"bool": {"must": [{"match_phrase": {"abstract": "ABSTRACT TRUNCATED"}}]}},
}

# Initialize the scroll
scroll = helpers.scan(
    os_connection, query=query, index=source_os_index, scroll="10m", size=10000
)
...
for batch_num, batch_ids in enumerate(
    chunked_iterable(ids_to_delete, batch_size), start=1
):
    query = {"query": {"terms": {"documentID": batch_ids}}}
